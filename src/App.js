...
// ── SEED DATA ──────────────────────────────────────────────
const SEED_CLIENTS = [
  {name:'Adriana Roa',country:'Asunción, Paraguay',start_date:'2026-02-19',end_date:'2026-11-19',duration_months:9,estado:'activo',color:'#4CAF50',leads:277,spend:458,objetivo:'Mínimo $5.000/mes',facturacion:'No especificada',problema:'Falta de cierres constantes'},
  {name:'Judy Rodriguez',country:'Asunción, Paraguay',start_date:'2026-03-18',end_date:'2026-06-18',duration_months:3,estado:'activo',color:'#5B9BD5',leads:423,spend:1635,objetivo:'No especificado',facturacion:'No especificada',problema:'No especificado'},
  ...
  {name:'Alejandro Nardini',country:'Buenos Aires, Argentina',start_date:'2026-05-18',end_date:'2026-08-18',duration_months:3,estado:'activo',color:'#26C6DA',leads:93,spend:515,objetivo:'No especificado',facturacion:'No especificada',problema:'No especificado'},
  ...
  {name:'Carlos Barón',country:'Salta, Argentina',start_date:'2026-06-01',end_date:'2026-11-01',duration_months:5,estado:'completo',color:'#9E9E9E',leads:22,spend:120,objetivo:'No especificado',facturacion:'No especificada',problema:'No especificado'},
]
...
export default function App() {
  ...
  const seedData = useCallback(async () => {
    ...
    if (insertedClients) {
      // Insert ventas linked to clients
    ...
    <div className="app">
      <header className="header">
        <div className="logo">
          ...
          <span className="logo-name">F&F Services</span>
        </div>
      ...
      <main>
        {tab === 'tasks' && <TasksView tasks={tasks} setTasks={setTasks} fetchAll={fetchAll} />}
  ...
  return (
    <div className="view">
      ...
      <div className="topbar">
        ...
        <button className="btn-add" onClick={() => setShowForm(!showForm)}>＋ Nueva tarea</button>
      </div>
        ...
        <div className="form-panel">
          <div className="form-grid">
            <div className="form-field" style={{gridColumn:'1/-1'}}>
              <label>Título *</label>
              <input value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder="Descripción de la tarea" />
            </div>
          </div>
          <div className="form-grid">
            <div className="form-field">
              ...
              <select value={form.owner} onChange={e=>setForm({...form,owner:e.target.value})}>
                <option value="ambos">Francesco y Felipe</option>
                <option value="fran">Francesco</option>
          ...
          <div className="form-actions">
            ...
            <button className="btn-save" onClick={addTask}>Guardar</button>
          </div>
        </div>
      )}
...
function ReportsView({ reports, fetchAll }) {
  ...
  return (
    <div className="view">
      <div className="topbar">
        <h2>Reportes de campañas</h2>
        <button className="btn-add" onClick={()=>setShowForm(!showForm)}>＋ Nuevo reporte</button>
      </div>
      ...
      {showForm && (
        <div className="form-panel">
          ...
          <div className="form-grid">
            <div className="form-field">
              ...
              <select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}>
                <option value="ok">OK — todo bien</option>
                <option value="warn">Atención — revisar</option>
                <option value="bad">Alerta — problema</option>
              </select>
            </div>
          ...
          <div className="form-actions">
            <button className="btn-cancel" onClick={()=>setShowForm(false)}>Cancelar</button>
            <button className="btn-save" onClick={addReport}>Guardar</button>
          </div>
        </div>
      )}
      ...
      <div className="list">
        ...
        {reports.map(r => {
          const cplClass = r.cpl > 15 ? 'bad' : r.cpl > 8 ? 'warn' : 'good'
          const sClass = {ok:'s-ok',warn:'s-warn',bad:'s-bad'}[r.status]
          const sLabel = {ok:'OK',warn:'Revisar',bad:'Alerta'}[r.status]
          return (
            <div key={r.id} className="report-card">
              <div className="report-top">
                ...
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <span className={`status-tag ${sClass}`}>{sLabel}</span>
              ...
              <div className="report-metrics">
                ...
                <div className="metric-cell"><div className={`metric-val ${cplClass}`}>${Number(r.cpl).toFixed(1)}</div><div className="metric-lbl">CPL</div></div>
...
function ClientsView({ clients, ventas, fetchAll }) {
  ...
  const clientVentas = (clientId) => ventas.filter(v => v.client_id === clientId)
  const totalComis = (clientId) => totalComisiones(clientVentas(clientId))
  const pct = (clientId) => Math.min(100, Math.round(totalComis(clientId) / GOAL * 100))
  ...
  const addClient = async () => {
    ...
    await supabase.from('clients').insert({...form, color})
  ...
  return (
    <div className="view">
      ...
      {renewals.map(c => (
        <div key={c.id} className="renewal-banner">
          ⚠️ <strong>{c.name}</strong> — programa vence en <strong>{daysLeft(c.end_date)} días</strong>. Gestionar renovación.
        </div>
      ))}

      <div className="topbar">
        <h2>Clientes activos</h2>
        <button className="btn-add" onClick={()=>setShowForm(!showForm)}>＋ Nuevo cliente</button>
      </div>
          ...
          <div className="form-grid cols3">
            ...
            <div className="form-field"><label>Estado</label>
              <select value={form.estado} onChange={e=>setForm({...form,estado:e.target.value})}>
                ...
                <option value="completo">Programa completo</option>
              </select>
            </div>
          </div>
          <div className="form-actions">
            <button className="btn-cancel" onClick={()=>setShowForm(false)}>Cancelar</button>
            <button className="btn-save" onClick={addClient}>Guardar</button>
          </div>
        </div>
      )}
              ...
              <div className="client-header" onClick={()=>setExpanded(isExpanded?null:c.id)}>
                ...
                <div className="client-header-right">
                  ...
                  <div className="progress-wrap">
                    <div className="progress-bar-bg"><div className="progress-bar-fill" style={{width:`${p}%`,background:bc}} /></div>
                    <div className="progress-label">{p}% de $10K</div>
                  </div>
              ...
              {isExpanded && (
                <div className="client-body">
                  <div className="client-metrics">
                    {[
                      {v:c.leads,l:'Leads'},
                      {v:'$'+Math.round(c.spend),l:'Pauta'},
                      {v:c.leads&&c.spend?(c.spend/c.leads).toFixed(1):'-',l:'CPL'},
                      {v:c.leads&&c.spend?(c.leads/c.spend).toFixed(1)+'x':'-',l:'ROAS'},
                      {v:'$'+comis.toLocaleString(),l:'Comisionado',color:'var(--green)'},
                      {v:'$'+Math.max(0,GOAL-comis).toLocaleString(),l:'Faltan',color:comis>=GOAL?'var(--green)':'var(--yellow)'},
                    ].map((m,i) => (
                      <div key={i} className="cm-cell">
                        <div className="cm-val" style={m.color?{color:m.color}:{}}>{m.v}</div>
                        <div className="cm-lbl">{m.l}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="guarantee-section">
                    <div className="guarantee-header">
                      <span>Progreso hacia la garantía de $10.000</span>
                      <span className="mono">${comis.toLocaleString()} / ${GOAL.toLocaleString()}</span>
                    </div>
                    ...
                    <div className="guarantee-footer">
                      <span>{c.start_date ? 'Inicio: '+c.start_date : ''}</span>
                  ...
                  <div className="ventas-section">
                    <div className="ventas-header">
                      <span className="ventas-title">Ventas y comisiones</span>
                      <button className="btn-venta" onClick={()=>setModalClient(c.id)}>＋ Registrar venta</button>
                    </div>
      ...
      {modalClient && (
        <div className="modal-overlay" onClick={()=>setModalClient(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="form-actions">
              <button className="btn-cancel" onClick={()=>setModalClient(null)}>Cancelar</button>
              <button className="btn-save" onClick={saveVenta}>Registrar</button>
            </div>
          </div>
        </div>
      )}
...
function PipelineView({ pipeline, clients, fetchAll }) {
  ...
  const filtered = filter === 'all' ? pipeline : pipeline.filter(p => p.stage === filter)
  const blocked = pipeline.filter(p => p.stage === 'esperando' && p.days_in_stage >= 7).length
  ...
  const advance = async (item) => {
    ...
    if (idx < STAGE_ORDER.length - 1) {
      await supabase.from('pipeline').update({stage: STAGE_ORDER[idx+1], days_in_stage: 0}).eq('id', item.id)
      fetchAll()
    }
  }
  ...
  return (
    <div className="view">
      ...
      <div className="topbar" style={{marginTop:16}}>
        ...
        <button className="btn-add" onClick={()=>setShowForm(!showForm)}>＋ Nueva campaña</button>
      </div>
      ...
      {showForm && (
        <div className="form-panel">
          ...
          <div className="form-grid cols3">
            <div className="form-field"><label>Etapa</label>
              <select value={form.stage} onChange={e=>setForm({...form,stage:e.target.value})}>
                {STAGE_ORDER.map(s => <option key={s} value={s}>{STAGE_LABELS[s]}</option>)}
              </select>
            </div>
            <div className="form-field"><label>Responsable</label>
              <select value={form.owner} onChange={e=>setForm({...form,owner:e.target.value})}>
                <option value="felipe">Felipe</option>
                <option value="gaston">Gastón</option>
                <option value="christian">Christian</option>
          ...
          <div className="form-actions">
            <button className="btn-cancel" onClick={()=>setShowForm(false)}>Cancelar</button>
            <button className="btn-save" onClick={addPipe}>Guardar</button>
          </div>
        </div>
      )}

      <div className="filters">
        {[['all','Todo'],['esperando','Bloqueadas'],['guiones','Guiones'],['edicion','Edición'],['lanzar','Para lanzar'],['activa','Activas']].map(([f,l]) => (
          <button key={f} className={`filter ${filter===f?'active':''}`} onClick={()=>setFilter(f)}>{l}</button>
        ))}
      </div>

      <div className="list">
        {filtered.length === 0 && <div className="empty">Sin campañas</div>}
        {filtered.map(p => {
          ...
          return (
            <div key={p.id} className={`pipe-card ${cardClass}`}>
              <div>
                <div className="pipe-name">{p.client_name}</div>
                <div className="pipe-prop">{p.property}</div>
              ...
              <div className="pipe-right">
                <div className={`pipe-days ${isBlocked?'warn':''}`}>{p.days_in_stage}d en etapa</div>
                <div className="pipe-owner">{OWNER_LABELS[p.owner]||p.owner}</div>
                <div style={{display:'flex',gap:4}}>
                  {p.stage !== 'activa' && <button className="btn-advance" onClick={()=>advance(p)} title="Avanzar etapa">›</button>}
...
function ContentView({ content, fetchAll }) {
  ...
  return (
    <div className="view">
      ...
      <div className="topbar">
        <h2>Banco de contenido orgánico</h2>
        <button className="btn-add" onClick={()=>setShowForm(!showForm)}>＋ Nueva idea</button>
      </div>
          ...
          <div className="form-grid cols3">
            ...
            <div className="form-field"><label>Formato</label>
              <select value={form.format} onChange={e=>setForm({...form,format:e.target.value})}>
                {['Reel','Carrusel','Historia','Improvisado'].map(f => <option key={f} value={f}>{f}</option>)}
              </select>
            </div>
          </div>
          <div className="form-actions">
            <button className="btn-cancel" onClick={()=>setShowForm(false)}>Cancelar</button>
            <button className="btn-save" onClick={addContent}>Guardar</button>
          </div>
        </div>
      )}
